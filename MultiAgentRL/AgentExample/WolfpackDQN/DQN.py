...
class ConvNet(object): 
	def __init__(self):
		...
		self.W_out = tf.Variable(tf.truncated_normal([128, 7],mean=0,stddev=0.02)) 
		self.b_out = tf.Variable(tf.constant(0.05, shape=[7]))
		...
		loss = tf.reduce_sum(tf.squared_difference(self.y,self.out))
		...
		self.W_out_t_holder = tf.placeholder(tf.float32, [128, 7])
		self.b_out_t_holder = tf.placeholder(tf.float32, [7])
		...
		self.saver = tf.train.Saver(max_to_keep=160)
