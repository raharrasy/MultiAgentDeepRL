...
class State(object) :
		...
		def sense(self,newRGB,ExperienceFlag=False,lastExpFlag=False,mode="FULL_IMAGE"):
			...
			if mode == "FULL_IMAGE":
				...
				if self.obsMode == "PARTIAL":
					player_coords = [(x,y) for (x,y) in player_coords if x>=top and x<bottom and y>=left and y<right]
					food_coords = [(x,y) for (x,y) in food_coords if x>=top and x<bottom and y>=left and y<right]
					obstacleCoord = [(x,y) for (x,y) in obstacleCoord if x>=top and x<bottom and y>=left and y<right]

				observations = obstacleCoord, player_coords, food_coords
				for player in self.player_list:
					player.sense(observations,ExperienceFlag=ExperienceFlag,LastExpFlag=lastExpFlag)
				for food in self.food_list:
					food.sense(observations,ExperienceFlag=ExperienceFlag,LastExpFlag=lastExpFlag)
